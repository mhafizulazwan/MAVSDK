        ...
        if (res != CURLE_OK) {
            ...
            for (auto& jpeg_path : json_obj["jpeg_storage_path"]) {
                ...
                if (res == CURLE_OK) {
                    ...
                    // Save the data
                    std::ofstream jpeg_file("/home/azwan/Dropbox/custom_drone_test/jpeg/" + filename, std::ios::binary);
            ...
            for (auto& tiff_path : json_obj["raw_storage_path"]) {
                ...
                if (res == CURLE_OK) {
                    ...
                    // Save the data
                    std::ofstream tiff_file("/home/azwan/Dropbox/custom_drone_test/tiff/" + filename, std::ios::binary);
...
size_t Micasense::VectorCallback(void* contents, size_t size, size_t nmemb, std::vector<unsigned char>* s) {
...
size_t Micasense::StringCallback(void* contents, size_t size, size_t nmemb, std::string* s) {
    size_t newLength = size * nmemb;
    try {
...
std::string Micasense::getCurrentTimestamp() {
